...
FedProx adds a proximal term to local training to reduce drift from the broadcast model~\cite{li2020fedprox}. SCAFFOLD uses server and client control variates to correct client drift~\cite{karimireddy2020scaffold}. In this implementation, SCAFFOLD local training uses SGD with momentum because the control-variate update assumes SGD-like parameter displacement. FedBN keeps batch-normalization statistics local while aggregating non-BatchNorm parameters~\cite{li2021fedbn}. Communication cost is counted as transmitted model-state bytes across all rounds; SCAFFOLD includes the additional control-variate traffic.
...
\begin{itemize}
    ...
    \item \textbf{Author contribution.} Abzal Orazbek implemented the experiments, prepared the analysis outputs, and drafted the manuscript. Artem Grents contributed to project framing and manuscript review.
\end{itemize}
